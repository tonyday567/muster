...
metaShard :: TChan MetaAction -> TQueue Text -> IO (Shard IO)
...
busSink :: Map Text (Text -> IO ()) -> IO (Shard IO)
busSink sinks = do
  ref <- newIORef []
  pure $
    shard
...
bucketShard :: FilePath -> IO (Shard IO)
...
diagShard :: TQueue Text -> IO (Shard IO)
diagShard diag = do
  ref <- newIORef []
  pure $
    shard
